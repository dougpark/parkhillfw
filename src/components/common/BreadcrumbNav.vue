...
<template>
  <nav class="flex flex-wrap items-center gap-1 text-sm text-[#444746]" aria-label="Breadcrumb">
    ...
    <template v-for="(crumb, index) in trail ?? []" :key="crumb.title">
      ...
      <RouterLink v-if="crumb.to" :to="crumb.to" class="rounded-full px-2 py-1 hover:bg-[#f0f4f9]">{{ crumb.title }}</RouterLink>
      <button v-else type="button" class="rounded-full px-2 py-1 hover:bg-[#f0f4f9]" @click="$emit('crumbClick', index)">{{ crumb.title }}</button>
    </template>
    <ChevronRight class="h-4 w-4" aria-hidden="true" />
    <span class="px-2 py-1 font-medium text-[#1f1f1f]">{{ current }}</span>
  </nav>
</template>
