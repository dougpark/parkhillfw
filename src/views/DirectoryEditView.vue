...
<template>
  <section class="space-y-6">
    ...
    <form v-else class="space-y-6" @submit.prevent="saveDirectory">
      <section class="rounded-3xl border border-theme-border bg-surface p-5 shadow-sm sm:p-6">
        <h3 class="text-lg font-semibold">Household</h3>
        <div class="mt-5 grid gap-4 sm:grid-cols-2">
          <label class="text-sm font-medium sm:col-span-2">Street address<input :value="household.streetAddress" readonly :class="`${inputClass} cursor-not-allowed bg-app-bg`" /></label>
          <p class="text-xs text-content-muted sm:col-span-2">Address corrections must be made by an administrator.</p>
          ...
          <label class="flex items-center gap-3 text-sm font-medium sm:col-span-2"><input v-model="household.securityMember" type="checkbox" class="h-5 w-5 accent-accent" /> Security member</label>
      ...
      <section class="rounded-3xl border border-theme-border bg-surface p-5 shadow-sm sm:p-6">
        <div class="flex items-center justify-between gap-4"><div><h3 class="text-lg font-semibold">Adult residents</h3><p class="mt-1 text-sm text-content-muted">Anyone in the household can update the other residents here.</p></div><button type="button" class="inline-flex items-center gap-1 rounded-full border border-accent px-3 py-1.5 text-sm font-medium text-accent" @click="addResident"><Plus class="h-4 w-4" /> Add</button></div>
        <div class="mt-5 space-y-5">
          <div v-for="(resident, index) in residents" :key="resident.id ?? `new-${index}`" class="rounded-2xl bg-app-bg p-4">
            <div class="flex justify-end"><button type="button" class="rounded-full p-1.5 text-danger hover:bg-danger-subtle" aria-label="Remove resident" title="Remove resident" @click="residents.splice(index, 1)"><Trash2 class="h-4 w-4" /></button></div>
            <div class="grid gap-4 sm:grid-cols-2">
              ...
              <label class="flex items-center gap-3 text-sm font-medium sm:col-span-2"><input v-model="resident.isPrimaryContact" type="checkbox" class="h-5 w-5 accent-accent" /> Primary contact</label>
      ...
      <section class="rounded-3xl border border-theme-border bg-surface p-5 shadow-sm sm:p-6">
        <div class="flex items-center justify-between gap-4"><div><h3 class="text-lg font-semibold">Children and dependents</h3><p class="mt-1 text-sm text-content-muted">Include service availability so neighbors can find help.</p></div><button type="button" class="inline-flex items-center gap-1 rounded-full border border-accent px-3 py-1.5 text-sm font-medium text-accent" @click="addChild"><Plus class="h-4 w-4" /> Add</button></div>
        <div class="mt-5 space-y-5">
          <div v-for="(child, index) in children" :key="child.id ?? `new-${index}`" class="rounded-2xl bg-app-bg p-4">
            <div class="flex justify-end"><button type="button" class="rounded-full p-1.5 text-danger hover:bg-danger-subtle" aria-label="Remove child" title="Remove child" @click="children.splice(index, 1)"><Trash2 class="h-4 w-4" /></button></div>
            <div class="grid gap-4 sm:grid-cols-2">
              ...
              <label class="flex items-center gap-3 text-sm font-medium"><input v-model="child.petSitting" type="checkbox" class="h-5 w-5 accent-accent" /> Pet sitting</label>
              <label class="flex items-center gap-3 text-sm font-medium"><input v-model="child.babysitting" type="checkbox" class="h-5 w-5 accent-accent" /> Babysitting</label>
            </div>
          </div>
        </div>
      </section>
